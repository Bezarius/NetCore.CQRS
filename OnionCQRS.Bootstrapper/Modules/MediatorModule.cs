...
using OnionCQRS.Services.Query;
    ...
    public class MediatorModule : Module
    {
        ...
        protected override void Load(ContainerBuilder builder)
        {
            ...
            builder.RegisterGenericDecorator(
                ...
                fromKey: "service-handlers",
                toKey: "pipeline-handlers");

            // Decorate All Pipelines with our Validator
            builder.RegisterGenericDecorator(
                typeof(ValidatorHandler<,>),
                typeof(IRequestHandler<,>),
                fromKey: "pipeline-handlers");    // The outermost decorator should not have a toKey

            // Special registration of our Automapper Handler
            builder.RegisterGeneric(
                typeof(AutoMapperQuery<,>)).AsSelf();

            builder.RegisterGeneric(
                typeof(AutoMapperQueryHandler<,>))
                .Named("service-handlers",
                typeof(IRequestHandler<,>)) // Because these are missed in the scan above, we have to manually name them for decoration
                .SingleInstance();

            // Special Registration of our Generic Query Handler
            builder.RegisterGeneric(
                typeof(GenericQuery<>)).AsSelf();

            builder.RegisterGeneric(
                typeof(GenericQueryHandler<>))
            ...
            builder.RegisterGeneric(
                typeof(PaginateQuery<>)).AsSelf();

            builder.RegisterGeneric(
                typeof(PaginateQueryHandler<>))
                .Named("service-handlers", typeof(IRequestHandler<,>)) // Because these are missed in the scan above, we have to manually name them for decoration
                .SingleInstance();
